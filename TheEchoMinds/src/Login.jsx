...
export default function LoginPage() {
  ...
  function navigateAfterAuthSuccess() {
    ...
    const lineItems = st?.lineItems;
    if (redirectTo === "/checkout") {
      const hasCart = Array.isArray(lineItems) && lineItems.length > 0;
      const hasProduct = product?.name || product?.amount;
      navigate("/checkout", {
        replace: true,
        state: hasCart ? { lineItems } : hasProduct ? { product } : {},
      });
      return;
    }
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        <SiteSubpageHeader
          leading={<h1 className="text-lg md:text-xl font-semibold tracking-wide text-white">Sign in</h1>}
        />
